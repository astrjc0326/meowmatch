...
export default function Introduce ({navigation}: any) {
  ...
  // const optionbtnOnPress = [tw`py-1 px-2 border rounded-md my-2`, {backgroudColor: '#e9f5f5'}]
  ...
  return (
    <View style={tw`flex-1 content-center items-center algin-center w-screen`}>
      ...
      <Text style={tw`text-center content-center w-full text-gray-400 text-2xl font-semibold my-10 pt-2`}>Introduce yourself!</Text>
      <Text style={tw`text-gray-600 my-2 text-base text-center`}>Fill out the rest of your details so meows know a little more about you.</Text>
    ...
    <Text style={tw`text-bold text-gray-400 text-lg`}>Birthday</Text>
      <TextInput placeholder='MM/DD/YYYY' keyboardType='numeric' placeholderTextColor='#DEDADA' maxLength={8} style={tw`h-12 text-lg my-1 py-1 px-2 border border-gray-400 rounded-lg`}></TextInput>
    <Text style={tw`text-bold text-gray-400 text-lg`}>Name</Text>
    <TextInput placeholder='Add your first name' placeholderTextColor="#DEDADA" maxLength={20} style={tw`h-12 text-lg my-1 py-1 px-2 border border-gray-400 rounded-lg`}>
    </TextInput>
